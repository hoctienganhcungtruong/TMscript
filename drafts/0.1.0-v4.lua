...
mainUIFrame.Size = UDim2.new(0, 380, 0, 420)
...
---------------------------------------------------------------------
-- UI HELPERS (RE-STYLED)
---------------------------------------------------------------------
local function CreateRowContainer(size)
    local row = Instance.new("Frame")
    row.Size = size or UDim2.new(1, 0, 0, 38)
...
---------------------------------------------------------------------
-- INPUTS + BUTTONS GENERATION (UNIFIED WIREFRAME)
---------------------------------------------------------------------

-- Walkspeed Row
local wsRow = CreateRowContainer()
local walkSpeedInput = CreateTextInput(wsRow, " WalkSpeed...", UDim2.new(0, 130, 1, 0))
local setWalkSpeedButton = CreateButton(wsRow, "SetWalkSpeed", "Set WalkSpeed", UDim2.new(1, -140, 1, 0), UDim2.new(0, 140, 0, 0))

-- JumpHeight Row
local jhRow = CreateRowContainer()
local jumpHeightInput = CreateTextInput(jhRow, " JumpHeight...", UDim2.new(0, 130, 1, 0))
local setJumpHeightButton = CreateButton(jhRow, "SetJumpHeight", "Set JumpHeight", UDim2.new(1, -140, 1, 0), UDim2.new(0, 140, 0, 0))

-- FOV Row
local fovRow = CreateRowContainer()
local fovInput = CreateTextInput(fovRow, " Field of View...", UDim2.new(0, 130, 1, 0))
local setFOVButton = CreateButton(fovRow, "SetFOV", "Set FOV", UDim2.new(1, -140, 1, 0), UDim2.new(0, 140, 0, 0))

-- Utility Toggles Rows
local rowUtil1 = CreateRowContainer()
local infiniteJumpButton = CreateButton(rowUtil1, "InfiniteJump", "Infinite Jump", UDim2.new(0, 169, 1, 0))
local antiAFKButton = CreateButton(rowUtil1, "AntiAFK", "Anti-AFK", UDim2.new(0, 169, 1, 0), UDim2.new(0, 179, 0, 0))

local rowUtil2 = CreateRowContainer()
local noclipButton = CreateButton(rowUtil2, "Btn_Noclip", "Noclip", UDim2.new(0, 169, 1, 0))
local instantInteractButton = CreateButton(rowUtil2, "InstantInteract", "Instant Interact", UDim2.new(0, 169, 1, 0), UDim2.new(0, 179, 0, 0))
...
local xInput = CreateTextInput(tpInputRow, " X", UDim2.new(0, 108, 1, 0))
local yInput = CreateTextInput(tpInputRow, " Y", UDim2.new(0, 108, 1, 0), UDim2.new(0, 118, 0, 0))
local zInput = CreateTextInput(tpInputRow, " Z", UDim2.new(0, 108, 1, 0), UDim2.new(0, 236, 0, 0))
