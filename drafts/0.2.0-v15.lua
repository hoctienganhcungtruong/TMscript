...
local function checkHasAnySelectedPlayer()
    for _, obj in ipairs(_G.PlayerCardObjects) do
        if obj.IsSelected and obj.IsSelected() then
            return true
        end
    end
    return false
...
toggleLockBtn.MouseButton1Click:Connect(function()
    ...
    if not lockCameraEnabled and not checkHasAnySelectedPlayer() then
        toggleLockBtn.Text = "❌ Chọn Player Trước!"
        toggleLockBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
        task.wait(1)
...
local function getClosestSelectedPlayerToCursor()
    ...
    if _G.FOVCircle and _G.FOVCircle.Visible then
        if closestPart then
            _G.FOVCircle.Color = Color3.fromRGB(255, 50, 50)
        else
            _G.FOVCircle.Color = Color3.fromRGB(0, 255, 150)
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if lockCameraEnabled then
        if not checkHasAnySelectedPlayer() then
