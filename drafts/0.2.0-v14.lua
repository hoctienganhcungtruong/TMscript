...
for _, partName in ipairs(r6Parts) do
    ...
    partRow.Active = false -- Vô hiệu hóa chặn click của container hàng
    ...
    -- Kiểm tra nếu CreateButton trả về table (ví dụ: {Button = ..., Stroke = ...})
    local realButton = typeof(btn) == "table" and btn.Button or btn
    
    if realButton and realButton:IsA("GuiObject") then
        realButton.ZIndex = 100 -- Đẩy ZIndex lên cao hẳn
        realButton.Active = true -- Bật active riêng cho nút bấm
        
        -- Hiệu ứng Rê chuột sáng nhẹ
        realButton.MouseEnter:Connect(function()
            if _G.TargetPartName ~= actualPartName then
                realButton.BackgroundColor3 = Theme.SurfaceAccent
            end
        end)
        
        realButton.MouseLeave:Connect(function()
            if _G.TargetPartName ~= actualPartName then
                realButton.BackgroundColor3 = Theme.Surface
            end
        end)
        
        -- SỰ KIỆN CLICK TRỰC TIẾP (Đã dọn sạch lỗi nghẽn nhờ Active = false ở khung cha)
        realButton.MouseButton1Click:Connect(function()
            _G.TargetPartName = actualPartName
            
            -- Cập nhật màu sắc lập tức cho tất cả các nút Part để biết đã chọn
            for pName, bInstance in pairs(partButtons) do
                local b = typeof(bInstance) == "table" and bInstance.Button or bInstance
                if pName == _G.TargetPartName then
                    ...
                    if b:FindFirstChildOfClass("UIStroke") then
                        b:FindFirstChildOfClass("UIStroke").Color = Theme.Accent
                    end
                else
        ...
        -- Lưu lại Instance nút thực tế vào danh sách
        partButtons[actualPartName] = realButton
    end
end

-- Cập nhật hiển thị mặc định cho nút đầu tiên (ví dụ: Head)
task.defer(function()
...
local function generateEnginePlayerCard(player, parentFrame)
    ...
    local function applyVisualEffect()
        if isSelected then
            ...
            TweenService:Create(card, Theme.TweenInfo, {BackgroundColor3 = Theme.SurfaceAccent}):Play()
            TweenService:Create(stroke, Theme.TweenInfo, {Color = Theme.Accent, Thickness = 1.8}):Play()
            TweenService:Create(displayName, Theme.TweenInfo, {TextColor3 = Theme.Accent}):Play()
        else
...
local function safeRefreshPlayerList()
    ...
    if _G.PlayerCardObjects then
        for _, obj in ipairs(_G.PlayerCardObjects) do
            if obj.Card and typeof(obj.Card) == "Instance" then 
                obj.Card:Destroy() 
            end
        end
    end
    ...
    if not playersCollapsible or not playersCollapsible.Content then 
        return 
    end

    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= localPlayer then
            local cardData = generateEnginePlayerCard(p, playersCollapsible.Content)
            table.insert(_G.PlayerCardObjects, cardData)
        end
    end
end

Players.PlayerAdded:Connect(safeRefreshPlayerList)
Players.PlayerRemoving:Connect(safeRefreshPlayerList)
