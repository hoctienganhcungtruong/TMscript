...
local function createCollapsible(parent, title)
    ...
    layout.Padding = UDim.new(0, 6) -- Tăng padding nhẹ cho giãn mắt
    layout.Parent = content

    local expanded = false

    local function updateSize()
        -- Lấy chiều cao thực tế của các UI bên trong, nếu bằng 0 thì dự phòng khoảng trống tối thiểu lúc mở rộng
        local targetContentHeight = layout.AbsoluteContentSize.Y
        if targetContentHeight == 0 and expanded then
            targetContentHeight = 40 -- Chiều cao dự phòng cơ bản để ép giao diện phải hiện ra
        end

        local height = expanded and targetContentHeight or 0

        -- Animate thanh nội dung bên trong
        TweenService:Create(
            content,
            Theme.TweenInfo,
            {
        ...
        -- Animate Khung bao ngoài (32px nút tiêu đề + 4px khoảng cách + nội dung bên dưới)
        TweenService:Create(
            container,
            Theme.TweenInfo,
            {
    ...
    -- Đăng ký sự kiện Click đóng mở gốc duy nhất
    button.MouseButton1Click:Connect(function()
        expanded = not expanded
        updateSize()
    end)

    -- LẮNG NGHE SỰ THAY ĐỔI: Khi danh sách người chơi (Player Cards) tăng/giảm hoặc thêm UI, 
    -- chiều cao tự động tính toán lại mà không cần phải click lại từ đầu.
    layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        updateSize()
    end)

    -- Chạy kiểm tra kích thước mặc định ban đầu (Đóng menu)
    task.defer(function()
        updateSize()
    end)
